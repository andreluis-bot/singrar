...
export function EventsView() {
  const { events, addEvent, removeEvent, joinEvent, leaveEvent, location, user } = useStore();
      ...
      <header className="p-6 shrink-0 bg-gradient-to-b from-[#0a192f] to-transparent">
        <div className="flex justify-between items-center mb-6">
          ...
          <button 
            ...
            className="bg-gradient-to-r from-[#64ffda] to-emerald-400 text-[#0a192f] p-3 rounded-2xl shadow-lg hover:shadow-[#64ffda]/20 transition-all hover:scale-105"
          >
      ...
      <div className="flex-1 overflow-y-auto px-6 pb-24 space-y-6 custom-scrollbar">
        {(events || []).length === 0 ? (
          ...
          (events || []).sort((a, b) => a.date - b.date).map((event) => (
            <div key={event.id} className="bg-gradient-to-br from-[#112240] to-[#0a192f] rounded-3xl border border-white/5 overflow-hidden shadow-xl group hover:border-white/10 transition-all">
              <div className="p-5 border-b border-white/5 flex justify-between items-start relative overflow-hidden">
                ...
                {event.organizer === userEmail && (
                  <button 
                    ...
                    className="text-[#8892b0] hover:text-red-400 p-2 bg-white/5 rounded-xl hover:bg-red-500/10 transition-colors relative z-10"
                  >
                    <Trash2 size={18} />
                  </button>
                )}
                  ...
                  <div className="flex gap-2">
                    {event.attendees.includes(userEmail) ? (
                      <button 
                        ...
                        className="flex items-center gap-2 px-5 py-2.5 bg-red-500/10 text-red-400 rounded-xl text-sm font-bold hover:bg-red-500/20 border border-red-500/20 transition-all hover:scale-105"
                      >
                      ...
                      <button 
                        ...
                        className="flex items-center gap-2 px-5 py-2.5 bg-[#64ffda]/10 text-[#64ffda] rounded-xl text-sm font-bold hover:bg-[#64ffda]/20 border border-[#64ffda]/20 transition-all hover:scale-105"
                      >
            ...
            >
                      ...
                      <button
                        ...
                        className={`flex flex-col items-center gap-2 p-3 rounded-2xl border transition-all hover:scale-105 active:scale-95 ${
                          newType === t 
                            ...
                            : "bg-white/5 text-[#8892b0] border-white/5 hover:bg-white/10 hover:text-white"
                        }`}
                      >
              ...
              <div className="flex gap-3 mt-8">
                <button 
                  ...
                  className="flex-1 py-3.5 rounded-xl bg-white/5 text-white font-bold hover:bg-white/10 transition-all border border-white/10 hover:scale-[1.02] active:scale-[0.98]"
                >
                  Cancelar
                </button>
                <button 
